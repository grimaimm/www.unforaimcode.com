...
const LessonList = ({ learn }) => {
  if (!learn || !learn.content || learn.content.length === 0) {
    return (
      <CommingSoonState message='We’re preparing new lessons. Check back soon!' />
    );
  }
      ...
      <div className='flex flex-col gap-3'>
        {learn.content.map((contentItem, index) => (
          <Link
            ...
            href={`/learn/${learn.slug}/${contentItem.slug}`}
            aria-label={`View Lessons: ${contentItem.title}`}
          >
            <Card
              className={clsx(
                'flex w-full cursor-pointer flex-row items-center justify-between border border-zinc-300 bg-gradient-to-bl px-5 py-4 transition-all duration-300 dark:border-zinc-800 dark:from-zinc-900 dark:to-zinc-950 lg:hover:scale-[101%]',
              )}
            >
              <div className='flex items-center gap-3'>
                <SubContentIcon size={18} className='hidden md:flex' />
                <h5 className='font-[15px]'>
                  Lessons {index + 0} : {contentItem.title}
                </h5>
              </div>
              <div className='hidden items-center gap-2 md:flex'>
                <Tooltip title='Language: Indonesian and English'>
                  <div
                    className={clsx(
                      'rounded-full bg-indigo-200 px-2 py-1 text-xs font-medium text-indigo-700 dark:bg-indigo-900 dark:text-indigo-200',
                    )}
                  >
                    ID - EN
                  </div>
                ...
                {learn.language && (
                  <Tooltip title={learn.language}>
                    {STACKS[learn.language]}
                  </Tooltip>
                )}
              </div>
            </Card>
