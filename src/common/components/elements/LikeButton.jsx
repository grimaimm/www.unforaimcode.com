...
export default function LikeButton({ blogId, initialLikes }) {
  ...
  const [likesByUser, setLikesByUser] = React.useState(initialLikes || 0);
  const [contentLikes, setContentLikes] = React.useState(initialLikes || 0);

  // Calculate progress as a percentage of 5
  const progress = (contentLikes % 5) * 20;
  const isMaxLikes = contentLikes % 5 === 0 && contentLikes > 0;

  // Fetch current likes from the server
  React.useEffect(() => {
    async function fetchLikes() {
      try {
        ...
        if (response.ok) {
          ...
          setContentLikes(data.likes); // Assume the API returns the total number of likes
          setLikesByUser(data.likesByUser); // Assume the API returns likes by the current user
        } else {
  ...
  const addLike = async () => {
    if (isLoading || contentLikes >= 100) return;
    ...
    try {
      ...
      if (response.ok) {
        ...
        setContentLikes(updatedData.likes);
        setLikesByUser(updatedData.likesByUser);
      } else {
    ...
    <div className='flex items-center space-x-4'>
      {isLoading ? (
        ...
        <button
          ...
          disabled={isMaxLikes}
        >
          <LikeButtonHeart likes={contentLikes} progress={progress} />
        </button>
      )}
      ...
      <div
        className={clsx(
          ...
          contentLikes === 0
            ...
            : 'bg-primary-300/50 dark:from-primary-300 dark:to-primary-400 transition-colors dark:bg-gradient-to-tr dark:bg-clip-text dark:text-transparent',
        )}
      >
...
function LikeButtonHeart({ likes, progress }) {
  return (
      ...
      <div className='absolute w-full text-center text-2xl'>
        <div
          className={clsx(
            ...
            likes % 5 === 0 && likes > 0 ? 'emoji-animate' : '',
          )}
        >
          🥳
        </div>
      ...
      >
        ...
        <g clipPath='url(#clip-path)'>
          ...
          <rect
            ...
            className={clsx(
              'dark:text-zinc-600',
              likes % 5 === 0 && likes > 0 ? 'text-white' : 'text-zinc-400',
            )}
          />
