...
const ImgComponent = ({ node, ...props }) => {
  ...
  return (
    <Image
      ...
      height={900}
      alt="image"
      className={`${isBlogRoute ? 'mt-1 mb-4' : ''} rounded-xl`}
      {...props}
    />
  );
};
...
const H3Component = ({ node, ...props }) => {
  ...
  return (
    <h3
      className={`${isBlogRoute ? 'mt-6 mb-2' : 'pt-4'} text-[18px] font-medium leading-snug dark:text-neutral-300`}
      {...props}
    />
  );
};
...
const BlockquoteComponent = ({ node, ...props }) => {
  ...
  return (
    <blockquote
      id={id}
      className={`${isBlogRoute ? 'my-6' : ''} rounded-br-2xl border-l-[5px] border-neutral-700 border-l-cyan-500 bg-neutral-200 py-3 pl-3 md:pl-6 font-sora text-lg font-medium text-cyan-800 dark:bg-neutral-800 dark:text-cyan-200`}
      {...props}
    />
  );
};
